...
import com.hellokoding.springboot.repository.PeopleRepository;
import com.hellokoding.springboot.repository.PersonVoteRepository;
import com.hellokoding.springboot.repository.VoteRepository;
...
public class HelloController {
    ...
    public Model hello(Model model,
        ...
        for (Vote vote : votes) {
            country.put(vote.getId(), "Описание выборов:" + vote.getDescription() + " Адрес выборов:" + vote.getAddress()
                    + " Дата выборов:" + formatter.format(vote.getDateTime()));
        }
        ...
        if (action.equals("findNamesakes")) {
            ...
            String htmlPushkin = "<span style=\"color:red;\">";
                ...
                for (Person pInternal : persons) {
                    if (!p.getId().equals(pInternal.getId()) && p.getSurName().toLowerCase().equals(pInternal.getSurName().toLowerCase())) {
                        if (!personCollection.contains(p)) {
                            ...
                            if (p.getSurName().toLowerCase().equals("пушкин")) {

                                if (!personCollection.contains(p))
                                    htmlPushkin += p.getName() + " " + p.getMiddleName() + " " + p.getSurName() + " Address:" + p.getAddress() +
                                            " Phone:" + p.getPhone() + " birthdate:" + formatter.format(p.getBirthdate()) + "<br>";
                            } else {
                                userList.add(p.getName() + " " + p.getMiddleName() + " " + p.getSurName() + " Address:" + p.getAddress() +
                                        " Phone:" + p.getPhone() + " birthdate:" + formatter.format(p.getBirthdate()));
                            }
            ...
            htmlPushkin += "</span>";
            model.addAttribute("redPushkin", htmlPushkin);
    ...
    public Model hello2(Model model,
        ...
        if (action.equals("addPerson")) {
            ...
            try {
                ...
                userList.add("Ошибка! Неверно задана дата рождения! Требуемый формат: ГГГГ-ММ-ДД");
            }
            ...
            if (userList.size() > 0) {//Ошибка
                ...
                for (Person p : personCollection) {
                    peopleMap.put(p.getId(), p.getName() + " " + p.getMiddleName() + " " + p.getSurName() + " Address:" + p.getAddress() +
                            " Phone:" + p.getPhone() + " birthdate:" + p.getBirthdate().toString());
                }
            ...
            userList.add(savedPerson.getName() + " " + savedPerson.getMiddleName() + " "
                    + savedPerson.getSurName() + " Address:" + savedPerson.getAddress() +
                    " Phone:" + savedPerson.getPhone() + " birthdate:" + savedPerson.getBirthdate().toString());
        ...
        if (action.equals("addVote")) {
            ...
            userList.add("Описание выборов:" + savedVote.getDescription() + " Адрес выборов:" + savedVote.getAddress()
        ...
        if (action.equals("addPersonVote")) {
            ...
            if (vote == null || person == null) {
                ...
                for (Vote vote2 : votes) {
                    country.put(vote2.getId(), "Описание выборов:" + vote2.getDescription() + " Адрес выборов:" + vote2.getAddress()
